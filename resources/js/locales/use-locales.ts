...
export function useTranslate(namespace?: string) {
    ...
    const updateDayjsLocale = useCallback((lang: string) => {
...
export function useLocaleDirectionSync() {
    ...
    const handleSync = useCallback(async () => {
        if (settings.state.direction !== i18n.dir(currentLang.value)) {
            settings.setState({ direction: i18n.dir(currentLang.value) });
        }

        if (i18n.resolvedLanguage !== currentLang.value) {
            await i18n.changeLanguage(currentLang.value);
        }
    }, [currentLang.value, i18n, settings]);
